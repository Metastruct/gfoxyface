--- Low-level OSC module for VRChat Face Tracking.
---
--- Parses incoming OSC bundles/messages from VRChat on a UDP socket,
...
local function poll(sock)
	return function()
		while true do
			...
			if not data then
				if ip ~= "timeout" then print("vrcft receive error:", ip) end
				return
			end
...
local function send(address, ...)
	...
	if not ok then print("vrcft send error:", err) end
	return true
end
